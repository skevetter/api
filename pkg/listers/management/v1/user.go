...
type UserLister interface {
	...
	List(selector labels.Selector) (ret []*managementv1.User, err error)
	// Get retrieves the User from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*managementv1.User, error)
	UserListerExpansion
}

// userLister implements the UserLister interface.
type userLister struct {
	listers.ResourceIndexer[*managementv1.User]
}

// NewUserLister returns a new UserLister.
func NewUserLister(indexer cache.Indexer) UserLister {
	return &userLister{listers.New[*managementv1.User](indexer, managementv1.Resource("user"))}
}
