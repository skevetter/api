...
type ClusterRoleTemplateLister interface {
	...
	List(selector labels.Selector) (ret []*managementv1.ClusterRoleTemplate, err error)
	// Get retrieves the ClusterRoleTemplate from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*managementv1.ClusterRoleTemplate, error)
...
type clusterRoleTemplateLister struct {
	listers.ResourceIndexer[*managementv1.ClusterRoleTemplate]
}
...
func NewClusterRoleTemplateLister(indexer cache.Indexer) ClusterRoleTemplateLister {
	return &clusterRoleTemplateLister{listers.New[*managementv1.ClusterRoleTemplate](indexer, managementv1.Resource("clusterroletemplate"))}
}
