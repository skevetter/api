...
type ClusterAccessLister interface {
	...
	List(selector labels.Selector) (ret []*managementv1.ClusterAccess, err error)
	// Get retrieves the ClusterAccess from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*managementv1.ClusterAccess, error)
...
type clusterAccessLister struct {
	listers.ResourceIndexer[*managementv1.ClusterAccess]
}
...
func NewClusterAccessLister(indexer cache.Indexer) ClusterAccessLister {
	return &clusterAccessLister{listers.New[*managementv1.ClusterAccess](indexer, managementv1.Resource("clusteraccess"))}
}
